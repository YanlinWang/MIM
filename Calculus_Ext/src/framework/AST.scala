...
object AST {
  ...
  case class MethDef(returnType: String, name: String, paras: List[(String, String)],
    ...
    def methodCheck(info: Info, thisType: String): Unit = {
      ...
      for (x <- update) assert(info.table.contains(x), TypeNotFound(x))
      for (p <- paras) assert(info.table.contains(p._1), TypeNotFound(p._1))
      for (x <- update) assert(info.subType(thisType, x), Message(thisType + " <: " + x + " does not hold."))
      if (returnExpr.isDefined) {
        val env: Map[String, String] = paras.map(p => p._2 -> p._1).toMap + ("this" -> thisType)
        for (x <- update) {
          val mtype = info.mtype(name, x)
          assert(mtype.isDefined, Message("mbody(" + name + ", " + x + ", " + x + ") = error."))
      ...
      for (x <- update) {
        val mostSpecific = info.mostSpecific(name, thisType, x)
        assert(mostSpecific.size == 1 && mostSpecific.head == x,
            Message("findOrigin(" + name + ", " + thisType + ", " + x + ") = error."))
      }
    }
  }
