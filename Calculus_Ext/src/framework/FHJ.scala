...
object FHJ {
  ...
  case class MethDef(returnType: String, name: String, paras: List[(String, String)],
    ...
    def methodCheck(env: Env): Boolean = {
      ...
      for (x <- update) {
        val mostSpecific = env.info.mostSpecific(name, thisType, x)
        if (mostSpecific.size != 1 || mostSpecific.head != x) return false
      }
      true
    }
    override def toString = {
      ...
      "  " + returnType + " " + name + "(" + parasStr + ") override " + update.reduce((a, b) => a + ", " + b) + body
    }
  }
        ...
        if (error) None else {
          val methodMap = is.map(x => {
            ...
            for (value <- updateMap.values) {
              if (value.distinct.size != value.size) error = true
              if (value.size > 1 && value.contains(x.name)) error = true
            }
