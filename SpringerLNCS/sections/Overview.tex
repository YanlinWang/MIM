...
\end{lstlisting}\vspace{3pt}
Since the dynamic type of the receiver is \lstinline|DrawableLoggingDeck|, calling \lstinline|shuffleAndDraw()| triggers the ambiguity. Therefore, we need to find another algorithm for method binding.
...
Now we can actually redefine \lstinline|Deck| as follows:
\vspace{3pt}\begin{lstlisting}
interface Deck {
  void draw() {...}
  void shuffleAndDraw() {
...
The above example reflects the general case, that is, all method calls with receiver ``\lstinline|this|'' need to be qualified with its enclosing interface name, so as to avoid potential conflicts. Actually in \MIM{}, we can still write ``\lstinline|shuffle();|'' and ``\lstinline|draw();|''
in \lstinline|Deck|, because \MIM{} automatically desugars those method calls to qualified ones, by its implicit semantic rules.
