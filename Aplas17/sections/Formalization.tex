...
a minimal subset of Java 8 interfaces following Featherweight Java style~\cite{Igarashi01FJ}.\bruno{Mention that it is done 
...
The method typing rule (T-METHOD) is more interesting, since the method can either be an original implementation or an update.
Besides static type-checking for the return expression,
we further use the helper function $\mostSpecific$ to ensure that the method update is legal. The formal definition is available in Section~\ref{sec:aux}.
...
\subsection{Auxilary Definitions}~\label{sec:aux}
