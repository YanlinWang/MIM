...
In this section, we present the formal model \MIM{}, short for 
\emph{\textbf{F}eatherweight \textbf{H}irarchical \textbf{J}ava} , based on
...
Fields and primitive types are not modelled as well.
...
expression. Such value is represents an object instance
...
\end{array}
\end{displaymath}
\caption{Syntax of name{}.}\label{fig:syntax}
\end{figure*}
...
Examples on $\mbody$ are shown in Figure~\ref{fig:examplesmbody} for a better understanding. Note that we use \lstinline|m| to denote an original method, and ``\lstinline|m|$\uparrow$\lstinline|A|'' for a partial override on \lstinline|A|. For each small example, the result
