...
\bruno{There is quite a bit of teminology that we need to be careful
  to introduce earlier in the paper. For example: ``original
  method''. We are also still using terminology such as ``method
  update'', which I think is now outdated.
We need to read the paper carefully enough to spot
  possible uses of terminology which may make sense to us, but 
we have not introduced properly.}
...
\end{figure*}\bruno{do not use the notation ``-'' in T-METHOD, or
...
\item If $e$ includes $\left(\angl{J}\emph{\kwnew}\;I()\right).K::m(\overline{\angl{I_E} e})$ as a subexpression,
    then $ \emph{\mbody}(m, I, K) = (J', \overline{I_X} \; \overline{x}, I'_E \; e_0) $ and 
         $\num{\overline{x}} = \num{\overline{e}}$ for some $\overline{I_X}, \overline{x}, I_E'$ and $e_0$.
\item If $e$ includes $\emph{\kwsuper}.K::m(\overline{\angl{I_E} e})$ as a subexpression,
