...
to create an object. \haoyuan{TODO: mention relation to Java 8 interfaces with default methods; and traits}
...
\end{lstlisting}\vspace{3pt} \haoyuan{TODO: drawObject}
...
introduces a lot of boilerplate.\haoyuan{TODO: drawbacks: hard to correctly maintain self-references[]}
...
\paragraph{IV. Method exclusion like traits.} Same reason as above. \haoyuan{TODO: put exclusion and renaming together}

\paragraph{V. Method renaming like traits.} This is probably what people do in most cases, \haoyuan{not supported by most languages; breaks subtyping} by simply renaming one to avoid conflicts.
...
neither static dispatch nor dynamic dispatch in languages like C++ does so.
 Therefore, we need to find another algorithm for method binding\haoyuan{method resolution}.
...
On the other hand, \lstinline|((I) e).m()| \haoyuan{explicit casts are not supported in our model}, or simply \lstinline|e.m()| where \lstinline|e| has static type \lstinline|I|, behaves the same as \lstinline|e.I::m()| in our model. Such a dispatch make uses of both the static type and the dynamic type of the receiver. Intuitively, the static type specifies one branch of the method to avoid ambiguity, and the dynamic type finds the latest version on that branch. It may still introduce ambiguity when there are multiple paths from the static type to the dynamic type, and those paths cause conflicts. To prevent this, we disallow this kind of conflict to ensure unambiguity. That is to say, we do not allow two methods to override a same base method when they cause conflicts. This is natural, as they are just two versions of the same operation, hence it is no longer an ``unintentional'' conflict in that way.
