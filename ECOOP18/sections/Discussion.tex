...
With this approach, branches \lstinline|A| and \lstinline|B| are merged in the sense that they share the same code, which can be separately updated in future interfaces. Another approach is to deeply merge the branches, with similar effect as introducing an intermediate interface \lstinline|AB| to explicitly merge the two branches. However, this approach is problematic because there is no clear mechanism of identifying and further updating the merged branches. This could be an interesting future work to explore.
