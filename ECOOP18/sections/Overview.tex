...
\begin{itemize}
	...
	\item It cannot have state. But it is possible to support state in future work.
\end{itemize}
...
\begin{itemize}
  ...
  delegation can be used by introducing two fields (or field methods) with the \yanlin{"field methods"?}
...
\subsubsection{Potential solutions/workarounds in existing languages} Unfortunately in all languages we know of 
...
hierarchical overriding feature is an important feature, involved in the algorithm of hierarchical dispatch.
...
In  Figure~\ref{fig:examplesmbody}, (a) is the base case for unintentional conflicts, namely the \wordfork{} inheritance. (b) uses overriding to explicitly merge the conflicting methods. (c) represents hierarchical overriding. (d) and (e) are two base cases of diamond inheritance in \MIM{} and the definition of each $C$ is rejected because $T$ is an ambiguous parent to $C$. (f) gives a common solution to the diamond as in Java or traits, which is to explicitly override $A.m$ and $B.m$ in $C$. In the last three examples, conflicting methods $A.m$ and $B.m$ should be viewed as
