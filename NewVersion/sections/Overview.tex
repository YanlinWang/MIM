...
\lstinline|DrawableSafeDeck| inherits two \lstinline|draw| method from
both \lstinline|SafeDeck| and \lstinline|Drawable|. 
Indeed, using the a reduction model similar to FJ~\cite{} (where static types tracked), the 
reduction steps would roughly be:
\begin{lstlisting}
new DrawableSafeDeck().shuffleAndDraw() 
...
Here are the reductions performed in \MIM{} to accept this call: 
\begin{lstlisting}
new DrawableSafeDeck().shuffleAndDraw() 
-> [..., (Deck)new DrawableSafeDeck().draw()]
-> [..., SafeDeck.draw()]
\end{lstlisting}
Notably, we make use of the branch information by adding the cast \lstinline|Deck|, since the type of this-reference (\lstinline|Deck|) specifies the branch to use unambiguously. 
...
\haoyuan{Add reduction steps for this example and illustrate it better.}
\yanlin{Added, please review}
