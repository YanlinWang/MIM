...
\begin{itemize}
  ...
  \item Every method represents a behaviour and requires a body for
    its implementation (like Java 8 default methods). 
...
interfaces cannot have state.\bruno{I rephrased this sentence}
...
\bruno{The code calls getStack(), but that method is not defined! 
If you want to present a compelling example, please make sure that 
the code makes sense! I suppose that getStack() would be an abstract method?
}
...
\noindent\textbf{Potential Fixes} There are several other workarounds
...
\begin{itemize}
  ...
  Qualified names are used to specify the branch to give the right
  implementation.\bruno{don't forget to readjust text with what we
    have learned about C++.}
\end{itemize}

\noindent\textbf{Our solution:} 
The feature we expect is called \emph{triangle inheritance}, that is
to say, when the conflicting methods serve for different behaviours,
we can allow them to coexist in the extending interface in a type-safe way.
Below is our code that shows the triangle
inheritance.
...
%%Again it is slightly different from our prototype as we do not model \lstinline|void| or statements;
...
\end{lstlisting}\vspace{3pt}
\bruno{Please ensure that the syntax we use in the examples, matches 
up with the syntax in the formalization. In particular make sure that
the syntax for the various types of method calls are the same.}
\bruno{You don't actually explain your code! Please explain what the
  code does, and what are the different types of calls
  involved. Introduce some terminology here, namely the names for the 
different types of calls.}

\noindent\textbf{A comparison with C++:} 
...
our work.\bruno{Please revise.}

\subsection{Problem 2: Dynamic Dispatch for Code Reuse}\label{subsec:problem2}
Dynamic dispatching is essential and widely used in Object-Oriented programming.
...
it still refers to the old \lstinline|draw| in
\lstinline|Deck|\bruno{Is this discussion up-to-date? Please revise
  the discussion in view of what we know about C++ now.}. Thus programmers have to copy the \lstinline|shuffleAndDraw| code in
