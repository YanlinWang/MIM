...
C++ it is possible to have similar solutions solutions to both
...
\paragraph{Our solution:} To solve this problem it is important to preserve both conflicting methods
during inheritance instead of merging them into a single method. That is to say, the compiler
needs to accept \lstinline|DrawableDeck|. To disambiguite method calls
we can use \emph{up-casts}, so as to specify the ``branch'' in the
