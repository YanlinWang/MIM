...
This section illustrates the problem of unintentional method conflicts for motivation,
...
problems one by one, and lead the discussion on possible workarounds and our solutions.
Here it is worth clarifying that problem 1 and 2 are the entrance to the general problem,
and languages like C++ can give nice solutions to both, hence we are not basically proposing
some novel algorithm on conflict resolution, but inspired to formalize a minimal calculus
on this feature with type soundness. However, for problem 3 there is no satisfactory approach
in existing languages, thus we propose our new feature for direct support.
...
the same name and parameter types, but they are totally designed for different functionalities with different semantics.
...
\paragraph{Problem:} How to resolve unintentional method conflicts and invoke the conflicting methods separately without
ambiguity.

\paragraph{Potential Fixes:} There are several other workarounds
...
\paragraph{Our solution:} It is important to preserve both conflicting methods
during inheritance instead of explicitly merging them. That is to say, the compiler
needs to accept \lstinline|DrawableDeck|. And then the disambiguation is done by
up-casts, so as to specify the branch we would like to call.
...
that we expect to invoke the \lstinline|draw| from branch \lstinline|Deck|.

We call this case the triangle inheritance, where two unintentionally conflicting methods
are accepted by multiple inheritance. Note that C++ supports this feature and also addresses the
ambiguity by up-casts. The code for the above example is similar. 


\subsection{Problem 2: Dynamic Dispatching}
It seems that the solution to problem 1 is straightforward and easy to achieve, but things can
...
\end{lstlisting}\vspace{3pt}
Here \lstinline|shuffleAndDraw| invokes \lstinline|draw| from its own enclosing type. Ideally,
...
for instance, when we have the UML in Figure~\ref{fig:drawablesafedeck} and the following code:
...
is surely dispatched. But then \lstinline|shuffleAndDraw| invokes ``\lstinline|this.draw()|'', and at this point, the receiver
...
like \lstinline|shuffleAndDraw| to invoke \lstinline|SafeDeck.draw| because they belong to the same branch. It
seems that we choose dynamic dispatch but also make use of static type information, since the type of this-reference (\lstinline|Deck|) specifies the branch. Note that in C++, if we put the \lstinline[language=c++]|virtual| keyword in front of every \lstinline|draw| method, that means
...
\paragraph{Problem:} How to ensure that the correct method is invoked, by disambiguating conflicting methods in a dynamically dispatched setting, and how to achieve such a formalized language model. \\

\paragraph{Our solution:} Inspired by C++, we have formalized this feature into a minimal core calculus,
