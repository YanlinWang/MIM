...
\lstinline|DrawableDeck|\bruno{Marco complained about this sentence,
  which is in the todo list. I've then complained that we should have
  the todos at the places where TODOs were meant. Now, the sentence
  remains unchanged. PLEASE go over the TODO list, and check whether
  you've fixed the issue or not, if not move the TODO to the correct
  location! }. To disambiguate method calls, 
...
From the perspective of \lstinline|DrawableSafeDeck|, the \lstinline|draw| method seems to be ambiguous since \lstinline|DrawableSafeDeck| inherits two \lstinline|draw| method from both \lstinline|SafeDeck| and \lstinline|Drawable|. But ideally we would
...
makes use of both the static type and the dynamic type of the receiver, so it seems to be a
...
\end{lstlisting}\vspace{3pt}
\bruno{Marco complined about this but this was not addressed either!!!}
