...
unintentionally conflicting methods to coexist in a class. C\# allows this by interface multiple inheritance and explicit method implementation, but there are people argue that in C\# you cannot do "multiple inheritance" \emph{from} interfaces, you can only \emph{implement} multiple interfaces. We will have more detailed discussion on this in Section~\ref{sec:relatedwork}.
C++ accepts the triangle inheritance and
resolves the ambiguity by specifying the expected path by \emph{upcasts}. But the
drawback is that these conflicting methods cannot be further overriden. 
\bruno{how about C\#? Say something about C\# here.}\yanlin{added C\#}
% However, C++ has
