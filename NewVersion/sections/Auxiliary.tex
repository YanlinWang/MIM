...
lead to ambiguity, and in order to ensure type soundness, both of them have to be rejected by the type checker. Our model guarantees this by the interface checking rule\textsc{(T-Intf)} in Figure~\ref{fig:typingrules}. 
...
We proceed to give the definition of auxiliary functions \mostSpecific{} and \mostSpecificOverride, which are two core functions for the method lookup algorithm. Generally,
...
Below is the algorithm:\bruno{used for what and where?}
