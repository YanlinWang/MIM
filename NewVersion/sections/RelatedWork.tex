...
the renaming feature helps to solve the unintentional method conflicts
...
coexist. In contrast, our work provides mechanisms that allows 
...
\noindent {\bf C++ model.}
C++ supports a very flexible inheritance model and allows programmers to choose either static dispatch or dynamic dispatch for method lookup.
It allows the existence of unintentional conflicts and uses static
dispatch to resolve them, as discussed in Section~\ref{sec:overview}. 
\bruno{if we are dropping the static dispatching discussion this text
  may be outdated.}
With virtual methods, dynamic dispatch is used and 
method lookup algorithm will find the most specific method definition.\yanlin{to revise: be consistant with introduction}
...
to that discussion here too.}

Although C++ supports hierarchical dispatching, it does not support
hierarchical overriding.  However, there is a possible
workaround\footnote{put the url here} that mimicks hierarchical
overriding.  We call it the "MiddleMan" approach, as shown in
...
interface implementations allows an interface to inherit multiple 
...
(2) there is only one level conflicting method implementations at the
...
FJ provides the standard model of formalizing a Java-like object-oriented languages and 
...
importance in our hierarchical dispatch algorithm, and it allows for a
more precise subject-reduction theorem as discussed in
Section~\ref{}\bruno{put reference and make sure that we discuss this topic}.
