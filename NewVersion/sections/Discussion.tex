...
Other languages may support more complicated behaviors.
For instance, in Java 8 when putting two identical abstract methods together by multiple inheritance, there is no conflict error. In Figure~\ref{fig:abstractdiamond}, we use italic $m$ to denote abstract methods. For both cases, the Java compiler automatically merges conflicting $m$ in $C$. In contrast, \MIM{} distinguishes $A.m$ and $B.m$ in triangle inheritance (left), and rejects the diamond inheritance (right). There are two reasons. Firstly, our formalization just treats abstract methods as concrete methods with an empty body, and that simplifies
the rules and the proof a lot. Secondly, we distinguish and treat differently on the conflicting methods, since they may represent different operations, even if they are abstract. Thus our model adopts a very conservative behavior rather than automatically merging 
methods. For the diamond case, as we have mentioned, it is actually an intentional conflict due to the same source $T$.
