...
Featherweight Java~\cite{Igarashi01FJ}. \MIM{} is a minimal core calculus that formalizes the core concept of hierarchical dispatch and overriding. The syntax, typing rules and small-step semantics.
...
auxiliary function $\canInstantiate(I)$ checks whether interface $I$
can be instantiated or not, and is formalized in Section~\ref{sec:auxdefs}. Since triangle inheritance accepts conflicting branches to coexist, the check requires that the most specific method is concrete for each method on each branch.
