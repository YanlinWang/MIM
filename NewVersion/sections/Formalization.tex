...
in an interface are either default methods or abstract methods. Default methods provide default implementations for methods, and abstract methods have no method body, which requires future implementation. 
...
Rule \textsc{(T-Intf)} defines the typing rule for interfaces. The
condition\bruno{Well, there are two conditions. I suggest you explain
  in a bit more detail the 2 conditions, since this rule seems to be
  the more complex one. Some examples may help.} \yanlin{controversial rule. lets discuss later} helps to ensure unambiguity and type soundness of the calculus. We will
