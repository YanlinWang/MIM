...
method.\haoyuan{Marco: rephrase.} If the interface that it overrides is exactly the enclosing
...
Object creation is represented by $\new I$.\haoyuan{add a footnote: $new I()\{\}$ in Java.} Fields and primitive types are not modelled in \MIM{}. 
...
Section~\ref{sec:auxdefs}.\haoyuan{rephrase. I=J in typing rules and avoid talking about dynamic type.}
...
Note that we accept implicit upcasts on the arguments.\haoyuan{Remove the last sentence; it is just subtyping in OO and
check this in the paper everywhere.}
...
and in the meanwhile implicit casts are applied, so that static types are recorded via annotations. Finally the return type $I_e$ is put in the front as an annotation.
