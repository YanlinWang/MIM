...
the search space: it not only includes the body of $I_0$, but also its super types.
For the general case, however, the hierarchical invocation $\mbody(m, I, J)$ finds ``the most specific $m$
above $I$ and along path/branch $J$''.\haoyuan{Make sure we mentioned this in Overview.}
...
are invoked recursively and produce a single value in
