...
\bruno{In general, please try to use latex paragraphs to add more
  structure and emphasize contents.}
...
implies that we can uniquely identify a method by its name.\yanlin{the terminology of original methods should have been proposed in Overview.}
\bruno{Then please do so.}
...
There are two typing rules for method invocation: \textsc{(T-Invk)}
...
\mbody{} is different to FJ\bruno{Use xspace to fix the spacing problem
  of macros like mbody}, which has two parameters. The first $I_0$
...
\bruno{Explanation for T-Anno is missing.}\yanlin{Added.}
